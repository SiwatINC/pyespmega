...
class ESPMega:
    ...
    input_chnaged_cb: function
    input_buffer = [0]*16
    humidity_buffer: float = None
    room_temperature_buffer: float = None
    def __init__(self, base_topic: str, mqtt: pahomqtt.Client, mqtt_callback: function = None, input_callback: function = None):
        self.mqtt = mqtt
        self.base_topic = base_topic
        self.mqtt.subscribe(f'{base_topic}/inputs/#')
        ...
        self.mqtt.publish_callback = self.handle_message
        self.request_state()
    ...
    def analog_write(self, pin: int, state: bool, value: int):
        self.mqtt.publish(f'{self.base_topic}/pwm/{"%02d"}/set/state'%id,"on" if state else "off")
        self.mqtt.publish(f'{self.base_topic}/pwm/{"%02d"}/set/value'%id, int(value))
    # def dac_write(self):
    #     pass
    # def dac_read(self):
    #     pass
    ...
    def handle_message(self, client: pahomqtt.Client, data, message: pahomqtt.MQTTMessage):
        if (message.topic.startswith(self.base_topic+"/inputs/")):
...
class ESPMega_standalone(ESPMega):
    def __init__(self, base_topic: str, mqtt_server: str, mqtt_port: int, mqtt_use_auth: bool = False,
                  mqtt_username: str = None, mqtt_password: str = None, mqtt_callback: function = None, 
                  input_callback: function = None):
